...
describe('ProductList', () => {
  ...
  it('The number of pagination should be the maximum returned in request', async () => {
    ...
    expect(screen.getAllByTestId('pagination-number-button').length).toEqual(mockProducts.total_pages);
  });
  ...
  it('The next button should move to next page', async () => {
    ...
    expect(screen.getByTestId('results-page-number').innerHTML).toBe('Showing page: 2');
  });
  ...
  it('The next button sould be disabled on last page', async () => {
    ...
    await waitFor(() => screen.getByTestId('results-page-number'));
    expect(screen.getByTestId('results-page-number').innerHTML).toBe(`Showing page: ${mockProducts.total_pages}`);
